...
public class Group {
	...
	public Group(String name, UUID owner, boolean disciplined,
		...
		for (PlayerType permission : PlayerType.values()) {
			List<UUID> list = db.getAllMembers(id, permission);
			for (UUID uuid : list) {
		...
		for (Group subgroup : GroupManager.getSubGroups(id)) {
	...
	public void addMember(UUID uuid, PlayerType type, boolean savetodb) {
		...
		if (savetodb) {
			...
			if (isMember(uuid, type)){
				db.removeMember(uuid, id);
			}
			db.addMember(uuid, id, type);
		}
	...
	public static boolean link(Group supergroup, Group subgroup, boolean saveToDb) {
		...
		if (saveToDb) {
			db.addSubGroup(supergroup.getGroupId(), subgroup.getGroupId());
		}
	...
	public static boolean unlink(Group supergroup, Group subgroup, boolean savetodb) {
		...
		if (savetodb){
			db.removeSubGroup(supergroup.getGroupId(), subgroup.getGroupId());
		}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof Group g))
			return false;
		return g.getGroupId() == this.getGroupId();
	}
}
